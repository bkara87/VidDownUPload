...
class MainWindow(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        lbl_log = ctk.CTkLabel(log_header, text="📋 Canlı İşlem Günlüğü / System Console", font=ctk.CTkFont(size=13, weight="bold"), text_color=COLOR_TEXT_MAIN)
        ...
        self.log(f"[{APP_NAME} v{APP_VERSION}] Sistem hazır. Bir video bağlantısı yapıştırarak indirmeye başlayabilirsiniz.")

    def _build_download_tab(self):
        tab = self.tab_download

        lbl_title = ctk.CTkLabel(tab, text="📥 Sosyal Medya Video İndirme Modülü", font=ctk.CTkFont(size=18, weight="bold"), text_color=COLOR_TEXT_MAIN)
        lbl_title.pack(anchor="w", padx=20, pady=(15, 5))

        lbl_desc = ctk.CTkLabel(tab, text="Instagram Reels/Posts, YouTube Shorts/Videos veya TikTok bağlantısını aşağıya yapıştırın.", font=ctk.CTkFont(size=13), text_color=COLOR_TEXT_MUTED)
        ...
        self.entry_url = ctk.CTkEntry(
            url_frame,
            placeholder_text="https://www.instagram.com/reel/... veya https://www.tiktok.com/@...",
            height=46,
            corner_radius=10,
        ...
        self.btn_download.pack(fill="x", padx=20, pady=(5, 10))

        self.progress_download = ctk.CTkProgressBar(tab, mode="indeterminate", height=8, corner_radius=4)
        self.progress_download.pack(fill="x", padx=20, pady=(0, 15))
        ...
        p1 = ctk.CTkFrame(cards_frame, fg_color="#0B101D", corner_radius=12, border_width=1, border_color="#1E293B")
        p1.pack(side="left", fill="both", expand=True, padx=(0, 8))
        ctk.CTkLabel(p1, text="📸 Instagram", font=ctk.CTkFont(size=14, weight="bold"), text_color="#E1306C").pack(pady=(12, 4))
        ctk.CTkLabel(p1, text="Reels & Post Videoları", font=ctk.CTkFont(size=11), text_color=COLOR_TEXT_MUTED).pack(pady=(0, 12))

        p2 = ctk.CTkFrame(cards_frame, fg_color="#0B101D", corner_radius=12, border_width=1, border_color="#1E293B")
        p2.pack(side="left", fill="both", expand=True, padx=4)
        ctk.CTkLabel(p2, text="▶️ YouTube", font=ctk.CTkFont(size=14, weight="bold"), text_color="#FF0000").pack(pady=(12, 4))
        ctk.CTkLabel(p2, text="Shorts & 4K Videolar", font=ctk.CTkFont(size=11), text_color=COLOR_TEXT_MUTED).pack(pady=(0, 12))

        p3 = ctk.CTkFrame(cards_frame, fg_color="#0B101D", corner_radius=12, border_width=1, border_color="#1E293B")
        p3.pack(side="left", fill="both", expand=True, padx=(8, 0))
        ctk.CTkLabel(p3, text="🎵 TikTok", font=ctk.CTkFont(size=14, weight="bold"), text_color="#00F2FE").pack(pady=(12, 4))
        ctk.CTkLabel(p3, text="Filigransız HD İndirme", font=ctk.CTkFont(size=11), text_color=COLOR_TEXT_MUTED).pack(pady=(0, 12))
    ...
    def _apply_preset(self, preset_name):
        if preset_name == "reels":
            ...
            self.log("✅ Instagram Reels şablonu uygulandı (Sağ Alt Maskeleme & Logo).")
        elif preset_name == "tiktok":
            self.chk_mask.select()
            self.combo_mask_pos.set("Sağ Alt (Instagram/TikTok)")
            self.chk_logo.select()
            self.combo_logo_pos.set("Sağ Üst")
            self.log("✅ TikTok şablonu uygulandı (Sağ Alt Maskeleme, Sağ Üst Logo).")
